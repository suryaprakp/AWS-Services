...
resource "aws_lambda_function" "datadog_autotagger" {
  ...
  filename         = "${path.module}/packages/datadog_autotagger_lambdafunction.zip"
  ...
  source_code_hash = "${base64sha256(file("${path.module}/packages/datadog_autotagger_lambdafunction.zip"))}"
  runtime          = "python3.7"
  timeout          = 80
